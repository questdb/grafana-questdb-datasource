	...
	t.Run("should parse settings correctly", func(t *testing.T) {
		...
		tests := []struct {
				...
				args: args{
					config: backend.DataSourceInstanceSettings{
						...
						JSONData: []byte(`{ "server": "test", "port": 8812, "username": "john", "timeout": 10, "queryTimeout": 50,
											"enableSecureSocksProxy": false, "tlsMode": "disable", "disablePreparedStatements": true,
			...
			},
			{
				...
				args: args{
					config: backend.DataSourceInstanceSettings{
						JSONData:                []byte(`{"server": "test", "username": "u", "port": "1234", "timeout": 15, "queryTimeout": 25, "maxOpenConnections": 10, "maxIdleConnections": 5, "maxConnectionLifetime": 3600, "disablePreparedStatements": "true"   }`),
			...
			{
				// Provisioning interpolates $VAR env references; an unset var yields
				// an empty string, which must mean the default (prepared statements on).
				name: "empty-string disablePreparedStatements means default",
				args: args{
					config: backend.DataSourceInstanceSettings{
						JSONData:                []byte(`{"server": "test", "username": "u", "port": 8812, "disablePreparedStatements": ""}`),
						DecryptedSecureJSONData: map[string]string{"password": "p"},
					},
				},
				expectedSettings: Settings{
