...
const DATASOURCE_NAME = `QuestDB-e2e-test-${uuidv4()}`;
let datasourceUID;
let apiToken;
...
export async function addDatasource(page) {
  try {
    ...
    const QuestDBDataSource = page.locator(`button[aria-label="${selectors.pages.AddDataSource.dataSourcePluginsV2('QuestDB')}"]`);
    await QuestDBDataSource.click();
