...
func e2eTests(cmd *cobra.Command, args []string) {
	...
	questDbAssets := buildPlugin(ctx, client)
	questDBContainer := startQuestDB(client, ctx)
	grafanaContainer := startGrafana(client, questDbAssets, questDBContainer)
...
func startGrafana(client *dagger.Client, questDbDist *dagger.Directory, questdbContainer *dagger.Container) *dagger.Container {
	fmt.Println("Building Grafana")
	container := client.
		Container().
		From("grafana/grafana:main").
		WithFile("/etc/grafana/grafana.ini", client.Host().File("e2e/custom.ini")).
		WithMountedDirectory("/var/lib/grafana/plugins/questdb-datasource", questDbDist).
		WithServiceBinding("questdb", questdbContainer.AsService()).
...
func startQuestDB(client *dagger.Client, ctx context.Context) *dagger.Container {
	fmt.Println("Starting QuestDB")
	container := client.Container().
		From("questdb/questdb:latest").
		WithExposedPort(9000)

	fmt.Println("QuestDB started")
