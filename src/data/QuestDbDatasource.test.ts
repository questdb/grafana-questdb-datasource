...
describe('QuestDBDatasource', () => {
  ...
  describe('applyTemplateVariables', () => {
    it('interpolates', async () => {
      ...
      const query = { rawSql: 'select', queryType: QueryType.SQL } as QuestDBQuery;
    ...
    it('should handle $__conditionalAll and not replace', async () => {
      const query = { rawSql: '$__conditionalAll(foo, $fieldVal)', queryType: QueryType.SQL } as QuestDBQuery;
  ...
  describe('Tag Keys', () => {
    it('should Fetch Default Tags When No Second AdHoc Variable', async () => {
      const spyOnReplace = jest.spyOn(templateSrvMock, 'replace').mockImplementation(() => '$questdb_adhoc_query');
      const ds = cloneDeep(mockDatasource);
      const frame = new ArrayDataFrame([{ name: 'column', type: 'string', table: 'table' }]);
      const spyOnQuery = jest.spyOn(ds, 'query').mockImplementation((_request) => of({ data: [frame] }));

      const keys = await ds.getTagKeys();
      expect(spyOnReplace).toHaveBeenCalled();
      const expected = { rawSql: 'select column_name, data_type, table_name from information_schema.columns' };
      ...
      expect(keys).toEqual([{ text: 'table.column' }]);
    });
  ...
  describe('fetchFieldsFull', () => {
    it('sends a correct query when only table name is provided', async () => {
      const ds = cloneDeep(mockDatasource);
      const frame = new ArrayDataFrame([{ column: 'column', type: 'string', designated: false }]);
      const spyOnQuery = jest.spyOn(ds, 'query').mockImplementation((_request) => of({ data: [frame] }));
      await ds.fetchFields('table_name');
      const expected = { rawSql: 'select "column", type, designated from table_columns(\'table_name\')' };
    ...
    it('sends a correct query when table name contains a dot', async () => {
      ...
      await ds.fetchFields('table.name');
      const expected = { rawSql: 'select "column", type, designated from table_columns(\'table.name\')' };
